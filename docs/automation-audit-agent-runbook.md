...
- **Stored template repair**: a one-time startup repair (`upgrade_legacy_scorecard_template_categories`) assigns canonical categories to templates that predate validation, so stored templates stay activatable instead of being rejected or scored as `uncategorized`. Generic/unlabeled legacy criteria (such as the pre-v2 Memory Audit Scorebook's `criterion_1`…`criterion_8`) map positionally: `criterion_1`–`criterion_3` → durable state correctness, `criterion_4`–`criterion_6` → retrieval or memory use, `criterion_7`–`criterion_8` → safety/private-information handling. Criteria whose id/label carries a recognizable keyword use the inference rules below. Historical run snapshots are intentionally left untouched.
