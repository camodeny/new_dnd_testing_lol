...
- **Template version 2**: new and updated templates require an explicit canonical category per criterion. Missing categories and values that match no canonical name or alias are rejected with a `400` at create and update, instead of being silently inferred or uncategorized. Alias values (e.g. `memory`, `narrative`) are accepted and normalized to their canonical name. A template containing uncategorized criteria cannot be attached to a scenario (activation is rejected with a `400`). The active Memory Audit Scorebook uses explicit categories for durable state correctness, retrieval or memory use, and safety/private-information handling. Runs store a snapshot, so later edits do not rewrite historical audits.
