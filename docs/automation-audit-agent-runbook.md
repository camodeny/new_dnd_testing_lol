...
### 4. Weight Defaults, Legacy Snapshots, and Category Validation
...
- **Template version 2**: new and updated templates require an explicit canonical category per criterion. Missing categories and values that match no canonical name or alias are rejected with a `400` at create and update, instead of being silently inferred or uncategorized. Alias values (e.g. `memory`, `narrative`) are accepted and normalized to their canonical name. A template containing uncategorized criteria cannot be attached to a scenario (activation is rejected with a `400`). Runs store a snapshot, so later edits do not rewrite historical audits.
- **Stored template repair**: a one-time startup repair (`upgrade_legacy_scorecard_template_categories`) assigns canonical categories to templates that predate validation, so stored templates stay activatable instead of being rejected or scored as `uncategorized`. The deployed pre-v2 Memory Audit Scorebook used generic `criterion_1`…`criterion_8` ids with real stored descriptions; it is repaired with an explicit per-criterion mapping based on those descriptions (`criterion_1`/`criterion_3`/`criterion_4`/`criterion_7`/`criterion_8` → durable state correctness, `criterion_2` → safety/private-information handling, `criterion_5`/`criterion_6` → retrieval or memory use). Other legacy criteria are repaired only when a semantic canonical category can be inferred from their id/label; unrelated generic criteria are left unchanged rather than assigned fabricated categories. Historical run snapshots are intentionally left untouched.
- **Legacy snapshots**: historical snapshots lacking `weight` or `category` remain readable through the scoring normalization path (`get_criterion_category()`), which is separate from template validation. Recognizable legacy ids/labels use the inference rules below; unrecognizable or invalid values are reported as `uncategorized` and produce a `scorecard_configuration.valid: false` signal instead of disappearing from category reporting. No historical score is silently dropped or recomputed by a template edit. The same `scorecard_configuration` signal is exposed in the run UI, the run scorecard endpoint, and the audit bundle.
