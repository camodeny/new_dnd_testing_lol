...
- **Template version 2**: new and updated templates normalize category aliases to canonical names. Criteria that cannot be inferred from a legacy semantic id/label are accepted for compatibility but flagged for repair before activation. The active Memory Audit Scorebook uses explicit categories for durable state correctness, retrieval or memory use, and safety/private-information handling. Runs store a snapshot, so later edits do not rewrite historical audits.
- **Legacy snapshots**: historical snapshots lacking `weight` or `category` remain readable. Recognizable legacy ids/labels use the inference rules below; unrecognizable or invalid values are reported as `uncategorized` instead of disappearing from category reporting. No historical score is silently dropped or recomputed by a template edit.
