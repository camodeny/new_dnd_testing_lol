...
class Profile(Base):
    ...
    def to_dict(self):
        return {
            ...
            or (self.email.split("@")[0] if self.email else "adventurer"),
            "email": self.email,
        }
...
class Campaign(Base):
    ...
    status: Mapped[str] = mapped_column(
        String(16), nullable=False, default="lobby", server_default="lobby"
    )
    ...
    content_boundaries: Mapped[dict] = mapped_column(
        JSONB, nullable=False, default=dict, server_default=text("'{}'")
    )
    loot_mode: Mapped[str] = mapped_column(
        String(32),
        nullable=False,
        default="frequent_gamble",
        server_default="frequent_gamble",
    )
...
class CampaignDomainEvent(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "campaign_id",
            "sequence",
            name="uq_campaign_domain_events_campaign_sequence",
        ),
    )
...
class Outbox(Base):
    ...
    aggregate_type: Mapped[str] = mapped_column(
        String(64), nullable=False, default="campaign", server_default="campaign"
    )
...
class WorkerExecution(Base):
    ...
    aggregate_type: Mapped[str | None] = mapped_column(
        String(64), nullable=True, default="campaign"
    )
    aggregate_id: Mapped[uuid.UUID | None] = mapped_column(
...
class OperationTrace(Base):
    ...
    narration_completed_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
    ...
    telemetry_dropped: Mapped[bool] = mapped_column(
...
class CampaignThread(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "thread_type IN ('campaign', 'private')", name="ck_campaign_threads_type"
        ),
        Index(
    ...
    thread_type: Mapped[str] = mapped_column(
        String(32), nullable=False, default="campaign", server_default="campaign"
    )
...
class CampaignThreadMember(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "thread_id", "user_id", name="uq_campaign_thread_members_identity"
        ),
    )

    thread_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("campaign_threads.id", ondelete="CASCADE"),
        primary_key=True,
    )
    user_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
...
class PlayerSubmissionSegment(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "submission_id", "position", name="uq_player_submission_segments_position"
        ),
        CheckConstraint(
            "position >= 0", name="ck_player_submission_segments_nonnegative_position"
        ),
        CheckConstraint(
            "segment_type IN ('ic', 'ooc')", name="ck_player_submission_segments_type"
        ),
    )
    ...
    submission_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("player_submissions.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
...
class DMStream(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "status IN ('streaming', 'completed', 'abandoned', 'failed')",
            name="ck_dm_streams_status",
        ),
    ...
    status: Mapped[str] = mapped_column(
        String(16), nullable=False, default="streaming", server_default="streaming"
    )
    audience: Mapped[str] = mapped_column(
        String(32), nullable=False, default="campaign", server_default="campaign"
    )
    ...
    first_chunk_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
    ...
    def to_dict(self, *, include_final: bool = True):
        return {
            ...
            "created_at": self.created_at.isoformat() if self.created_at else None,
            "updated_at": self.updated_at.isoformat() if self.updated_at else None,
            "first_chunk_at": self.first_chunk_at.isoformat()
            if self.first_chunk_at
            else None,
            "last_chunk_at": self.last_chunk_at.isoformat()
...
class DMStreamChunk(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "stream_id", "sequence", name="uq_dm_stream_chunks_stream_sequence"
        ),
        CheckConstraint(
            "sequence >= 0", name="ck_dm_stream_chunks_nonnegative_sequence"
        ),
    )
    ...
    stream_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("dm_streams.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    sequence: Mapped[int] = mapped_column(Integer, nullable=False)
...
class DmTurn(Base):
    ...
    __table_args__ = (
        Index(
            "ix_dm_turns_campaign_thread_status", "campaign_id", "thread_id", "status"
        ),
    ...
    input_set_revision: Mapped[int] = mapped_column(
        Integer, nullable=False, default=1, server_default="1"
    )
    # Canonical ordered list of submission UUID strings included in the current input set.
    submission_ids: Mapped[list | None] = mapped_column(
    ...
    assembly_window_start: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
    assembly_window_end: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
    streaming_started_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
    resolved_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True
    ...
    def to_dict(self):
        return {
            ...
            "current_attempt_id": str(self.current_attempt_id)
            if self.current_attempt_id
            else None,
            "streaming_attempt_id": str(self.streaming_attempt_id)
            if self.streaming_attempt_id
            else None,
            "assembly_window_start": self.assembly_window_start.isoformat()
            if self.assembly_window_start
            else None,
            "assembly_window_end": self.assembly_window_end.isoformat()
            if self.assembly_window_end
            else None,
            "streaming_started_at": self.streaming_started_at.isoformat()
            if self.streaming_started_at
            else None,
            "resolved_at": self.resolved_at.isoformat() if self.resolved_at else None,
...
class DmTurnAttempt(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "turn_id", "attempt_number", name="uq_dm_turn_attempts_turn_number"
        ),
    ...
    turn_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("dm_turns.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    ...
    parent_attempt_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("dm_turn_attempts.id", ondelete="SET NULL"),
        nullable=True,
    )
    worker_job_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("worker_executions.id", ondelete="SET NULL"),
        nullable=True,
        index=True,
    )
...
class Character(Base):
    ...
    system: Mapped[str] = mapped_column(
        String(32), nullable=False, default="dnd5e"
    )  # dnd5e | pf2e | etc.
    name: Mapped[str] = mapped_column(String(128), nullable=False)
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now(), nullable=False
...
class Dnd5eCharacterSheet(Base):
    ...
    char_class: Mapped[str | None] = mapped_column(
    ...
    acrobatics_prof: Mapped[bool] = mapped_column(
        Boolean, nullable=False, default=False
    )
    animal_handling_prof: Mapped[bool] = mapped_column(
        Boolean, nullable=False, default=False
    )
    ...
    intimidation_prof: Mapped[bool] = mapped_column(
        Boolean, nullable=False, default=False
    )
    investigation_prof: Mapped[bool] = mapped_column(
        Boolean, nullable=False, default=False
    )
    ...
    perception_prof: Mapped[bool] = mapped_column(
        Boolean, nullable=False, default=False
    )
    performance_prof: Mapped[bool] = mapped_column(
        Boolean, nullable=False, default=False
    )
    persuasion_prof: Mapped[bool] = mapped_column(
        Boolean, nullable=False, default=False
    )
    religion_prof: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False)
    sleight_of_hand_prof: Mapped[bool] = mapped_column(
        Boolean, nullable=False, default=False
    )
    ...
    skill_expertise: Mapped[dict | None] = mapped_column(
        JSONB, nullable=True
    )  # e.g. {"stealth": true, "perception": true}
    ...
    skills: Mapped[list | None] = mapped_column(
        JSONB, nullable=True
    )  # [{"skill_name":"Stealth","is_proficient":true,"is_expertise":false}]
    saving_throws: Mapped[list | None] = mapped_column(
        JSONB, nullable=True
    )  # [{"ability":"dexterity","is_proficient":true}]

    passive_perception: Mapped[int | None] = mapped_column(
        Integer, nullable=True
    )  # override, else 10+WIS+prof
    ...
    speed_details: Mapped[str | None] = mapped_column(
    ...
    hit_dice: Mapped[str | None] = mapped_column(
        String(32), nullable=True
    )  # e.g. "1d8"
    hit_dice_total: Mapped[str | None] = mapped_column(
        String(32), nullable=True
    )  # e.g. "3d8"
    ...
    encumbrance_status: Mapped[str | None] = mapped_column(
        String(32), nullable=True
    )  # frontend alias for encumbrance

    # ── Attacks & Spellcasting ─────────────────────────────────────────
    attacks: Mapped[list | None] = mapped_column(
        JSONB, nullable=True
    )  # [{"name":"Longsword","bonus":5,"damage":"1d8+3","type":"slashing"}]
    weapons: Mapped[list | None] = mapped_column(
        JSONB, nullable=True
    )  # frontend alias: same shape as attacks, kept separate for round-trip
    ...
    spell_slots: Mapped[dict | None] = mapped_column(
        JSONB, nullable=True
    )  # {"1": {"max":4,"used":1}, ...} canonical frontend shape
    spells: Mapped[list | None] = mapped_column(
        JSONB, nullable=True
    ...
    features_and_traits: Mapped[str | None] = mapped_column(
        Text, nullable=True
    )  # class/racial features
    features: Mapped[list | None] = mapped_column(
        JSONB, nullable=True
    )  # frontend structured features[]
    proficiencies: Mapped[list | None] = mapped_column(
        JSONB, nullable=True
    )  # frontend proficiencies[]
    other_proficiencies_languages: Mapped[str | None] = mapped_column(
        Text, nullable=True
    )
    proficiencies_languages_json: Mapped[list | None] = mapped_column(
        JSONB, nullable=True
    )
    ...
    def to_dict(self):
        ...
        base["character_id"] = (
            str(base["character_id"]) if base.get("character_id") else None
        )
        ...
        # encumbrance alias
        base["encumbrance_status"] = base.get("encumbrance_status") or base.get(
            "encumbrance"
        )
        # weapons alias
