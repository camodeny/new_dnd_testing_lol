...
class ContextRecord(StrictModel):
    ...
    def _private_scope_is_explicit(self) -> "ContextRecord":
        if self.visibility == "private" and not (
            ...
            raise ValueError("private records require thread_ids or user_ids authorization")
        return self
...
class ForwardDmContextPacket(StrictModel):
    ...
    def _all_lanes_once_in_order(self) -> "ForwardDmContextPacket":
        ...
        if names != list(LANE_ORDER):
            raise ValueError("context packet must contain every named lane exactly once in canonical order")
        return self
        ...
        for lane in self.lanes:
            ...
            for record in lane.records:
                if record.use != "narration_eligible" or record.visibility == "dm_only":
                    continue
                if record.visibility == "private" and self.audience.audience != "private":
                    continue
...
def assemble_context_packet(
    *,
    ...
    lane_status: Mapping[LaneName | str, Literal["authoritative", "not_applicable", "unavailable"]] | None = None,
    ...
    for name in LANE_ORDER:
        ...
        for record in supplied:
            if _authorized(record, audience):
                ...
                decision = BudgetDecision(
                    lane=name, record_id=record.record_id, action="failed",
                    reason="required_record_not_authorized_for_attempt_audience",
                    estimated_bytes=_size(record.model_dump(mode="json")), required=True,
                )
                ...
                decisions.append(BudgetDecision(
                    lane=name, record_id=record.record_id, action="omitted",
                    reason="not_authorized_for_attempt_audience",
                    estimated_bytes=_size(record.model_dump(mode="json")), required=False,
                ))
        ...
        lane = ContextLane(
            name=name, authority_status=status, required=required_lane,
            records=accepted, source_errors=errors,
        )
        ...
        if lane_limit is not None:
            while _lane_size(lane) > lane_limit:
                ...
                if not removable:
                    decision = BudgetDecision(
                        lane=name, action="failed", reason="required_lane_exceeds_lane_budget",
                        estimated_bytes=_lane_size(lane), required=True,
                    )
                ...
                decisions.append(BudgetDecision(
                    lane=name, record_id=victim.record_id, action="omitted",
                    reason="lane_budget_pressure", estimated_bytes=_size(victim.model_dump(mode="json")),
                    required=False,
                ))
        lanes.append(lane)
    ...
    while _payload_size(audience, lanes) > budget.effective_max_bytes:
        candidates = [
            (lane, record) for lane in lanes for record in lane.records if not record.required
        ]
        if not candidates:
            actual = _payload_size(audience, lanes)
            decision = BudgetDecision(
                lane=LaneName.TURN_IDENTITY, action="failed",
                reason="required_packet_exceeds_total_budget", estimated_bytes=actual, required=True,
            )
            raise ContextBudgetError(
        ...
        lane, victim = min(
            candidates,
            key=lambda pair: (
                pair[1].priority, -LANE_ORDER.index(pair[0].name),
                pair[1].sort_key, pair[1].record_id,
            ),
        )
        lane.records.remove(victim)
        decisions.append(BudgetDecision(
            lane=lane.name, record_id=victim.record_id, action="omitted",
            reason="total_budget_pressure", estimated_bytes=_size(victim.model_dump(mode="json")),
            required=False,
        ))

    for lane in lanes:
        omitted_count = sum(1 for decision in decisions if decision.lane == lane.name and decision.action == "omitted")
        for record in lane.records:
            decisions.append(BudgetDecision(
                lane=lane.name, record_id=record.record_id, action="included",
                reason="required_authority" if record.required else "within_budget",
                estimated_bytes=_size(record.model_dump(mode="json")), required=record.required,
            ))
    ...
    metrics = [
        LaneMetric(
            ...
            omitted_records=sum(1 for d in decisions if d.lane == lane.name and d.action == "omitted"),
    ...
    structured_log(
        logger, logging.INFO, "forward_dm_context_assembled",
        campaign_id=audience.campaign_id, thread_id=audience.thread_id,
        audience=audience.audience, assembly_ms=round(packet.observability.assembly_ms, 3),
        serialized_bytes=serialized_bytes, estimated_tokens=packet.observability.estimated_tokens,
        lane_bytes={metric.lane.value: metric.bytes for metric in metrics},
        omitted=[f"{d.lane.value}:{d.record_id}" for d in decisions if d.action == "omitted"],
        source_versions=sorted({version for metric in metrics for version in metric.source_versions}),
        retrieval_dependencies=packet.observability.retrieval_dependencies,
    )
    return packet


def _scope(campaign_id: uuid.UUID, *, thread_ids: Iterable[str] = (), user_ids: Iterable[str] = ()) -> AuthorizationScope:
    return AuthorizationScope(
        campaign_id=str(campaign_id), thread_ids=list(thread_ids), user_ids=list(user_ids)
    )


def _source(source_type: str, source_id: Any, version: Any, revision: int | None, **provenance) -> SourceRef:
    return SourceRef(
        source_type=source_type, source_id=str(source_id), source_version=str(version),
        campaign_revision=revision, provenance=provenance,
    )


def _audience_for_attempt(db: Session, campaign: Campaign, turn: DmTurnAttempt) -> ContextAudience:
    thread = db.get(CampaignThread, uuid.UUID(turn.thread_id))
    if thread is None or thread.campaign_id != campaign.id:
        raise ContextAuthorizationError("Attempt thread is missing or belongs to another campaign")
    if turn.audience != thread.thread_type:
        raise ContextAuthorizationError("Attempt audience does not match its authoritative thread type")
    if thread.thread_type == "private":
        user_ids = list(db.scalars(
            select(CampaignThreadMember.user_id).where(CampaignThreadMember.thread_id == thread.id)
        ).all())
    else:
        user_ids = list(db.scalars(
            select(CampaignMember.user_id).where(CampaignMember.campaign_id == campaign.id)
        ).all())
        user_ids.append(campaign.owner_id)
    return ContextAudience(
        campaign_id=str(campaign.id), thread_id=str(thread.id),
        audience=thread.thread_type, user_ids=[str(user_id) for user_id in user_ids],
    )
...
def _sheet_value(sheet: Dnd5eCharacterSheet) -> dict[str, Any]:
    ...
    return {
        ...
        "abilities": {name: getattr(sheet, name) for name in (
            "strength", "dexterity", "constitution", "intelligence", "wisdom", "charisma"
        )},
        "conditions": sheet.conditions or [],
        "death_saves": {"successes": sheet.death_save_successes, "failures": sheet.death_save_failures},
        "exhaustion_level": sheet.exhaustion_level,
        "hit_points": {
            "current": sheet.hit_points_current, "maximum": sheet.hit_points_max,
...
def assemble_attempt_context(
    ...
    *,
    supplemental_records: Mapping[LaneName | str, Iterable[ContextRecord]] | None = None,
    supplemental_status: Mapping[LaneName | str, Literal["authoritative", "not_applicable", "unavailable"]] | None = None,
    ...
    if turn is None or campaign is None:
        raise MissingAuthoritativeContextError("Attempt's turn or campaign authority is missing")
    if (
        ...
        raise MissingAuthoritativeContextError("Attempt is stale or inconsistent with the current logical turn")
    ...
    records[LaneName.TURN_IDENTITY].append(ContextRecord(
        record_id=f"attempt:{attempt.id}", required=True, priority=100,
        value={
            "attempt_id": str(attempt.id), "turn_id": str(turn.id),
            "attempt_number": attempt.attempt_number, "input_set_revision": attempt.input_set_revision,
            "source_campaign_revision": attempt.source_revision, "submission_ids": list(attempt.submission_ids or []),
        },
        sources=[_source("dm_turn_attempt", attempt.id, attempt.input_set_revision, attempt.source_revision,
                         turn_id=str(turn.id), attempt_number=attempt.attempt_number)],
        authorization=scope, use="adjudication_only",
    ))
    ...
    submissions = list(db.scalars(
        select(PlayerSubmission).where(PlayerSubmission.id.in_(submission_ids)).order_by(PlayerSubmission.sequence)
    ).all()) if submission_ids else []
    if [submission.id for submission in submissions] != submission_ids:
        raise MissingAuthoritativeContextError("One or more exact attempt submissions are missing or out of order")
    character_ids: set[uuid.UUID] = set()
    for submission in submissions:
        if (
            submission.campaign_id != campaign.id or submission.thread_id != attempt.thread_id
            or submission.audience != attempt.audience
        ):
            raise ContextAuthorizationError(f"Submission {submission.id} is outside the attempt audience")
        ...
        if not segments:
            raise MissingAuthoritativeContextError(f"Submission {submission.id} has no typed IC/OOC segments")
        if [segment.position for segment in segments] != list(range(len(segments))):
            raise MissingAuthoritativeContextError(f"Submission {submission.id} segments are not contiguous")
        if submission.character_id:
            character_ids.add(submission.character_id)
        records[LaneName.PLAYER_INPUTS].append(ContextRecord(
            record_id=f"submission:{submission.id}", required=True, priority=100,
            sort_key=f"{submission.sequence:020d}",
            value={
                "submission_id": str(submission.id), "sequence": submission.sequence,
                "user_id": str(submission.user_id),
                "character_id": str(submission.character_id) if submission.character_id else None,
                "segments": [
                    {"position": segment.position, "segment_type": segment.segment_type, "text": segment.text}
                    for segment in segments
                ],
            },
            sources=[_source("player_submission", submission.id, submission.sequence, attempt.source_revision,
    ...
    for character_id in sorted(character_ids, key=str):
        ...
        if character is None:
            raise MissingAuthoritativeContextError(f"Protected player character {character_id} is missing")
        ...
        records[LaneName.PROTECTED_PCS].append(ContextRecord(
            record_id=f"pc-control:{character.id}", required=True, priority=100,
            value={
                "character_id": str(character.id), "name": character.name,
                "owner_user_id": str(character.owner_id), "control_policy": "player_only",
                "dm_may_not_choose_actions": True,
            },
            sources=[_source("character", character.id, character.updated_at.isoformat(), attempt.source_revision,
                             owner_id=str(character.owner_id))],
            authorization=scope, use="adjudication_only",
        ))
        sheet = db.scalar(select(Dnd5eCharacterSheet).where(Dnd5eCharacterSheet.character_id == character.id))
        if character.system == "dnd5e" and sheet is None:
            raise MissingAuthoritativeContextError(f"Relevant PC {character.id} has no authoritative D&D 5e sheet")
        if sheet is not None:
            records[LaneName.CHARACTER_STATE].append(ContextRecord(
                record_id=f"character-state:{character.id}", required=True, priority=100,
                value={"character_id": str(character.id), "system": character.system, "state": _sheet_value(sheet)},
                sources=[_source("dnd5e_character_sheet", sheet.id, sheet.updated_at.isoformat(),
                                 attempt.source_revision, character_id=str(character.id))],
                authorization=scope, use="adjudication_only",
            ))
        ruleset_id = f"ruleset:{character.system}"
        existing_ruleset = next(
            (record for record in records[LaneName.RULESET_IDENTITY] if record.record_id == ruleset_id), None
        )
        ruleset_source = _source(
            "character", character.id, character.updated_at.isoformat(), attempt.source_revision
        )
        if existing_ruleset is None:
            records[LaneName.RULESET_IDENTITY].append(ContextRecord(
                record_id=ruleset_id, required=True, priority=100,
                value={"system": character.system}, sources=[ruleset_source],
                authorization=scope, use="adjudication_only",
            ))
        else:
    ...
    if recent_event_limit:
        recent_events = list(db.scalars(
            ...
            .where(CampaignDomainEvent.campaign_id == campaign.id,
                   CampaignDomainEvent.sequence <= attempt.source_revision)
            .order_by(CampaignDomainEvent.sequence.desc())
            .limit(recent_event_limit)
        ).all())
        for event in reversed(recent_events):
            visibility = event.visibility if event.visibility in {"public", "campaign", "private", "dm_only"} else "dm_only"
            event_thread = None
            ...
            if visibility == "private" and event_thread is None:
                continue
            records[LaneName.RECENT_HISTORY].append(ContextRecord(
                record_id=f"domain-event:{event.id}", priority=80, sort_key=f"{event.sequence:020d}",
                value={
                    "event_id": str(event.id), "sequence": event.sequence, "event_type": event.event_type,
                    "payload": event.payload, "targets": event.targets,
                },
                sources=[_source("campaign_domain_event", event.id, event.sequence, event.sequence,
                                 operation_id=event.operation_id, trace_id=event.trace_id,
                                 upstream_provenance=event.provenance or {})],
                authorization=_scope(campaign.id, thread_ids=[event_thread] if event_thread else []),
                visibility=visibility,
                use="adjudication_only" if visibility == "dm_only" else "narration_eligible",
            ))
    ...
    statuses: dict[LaneName, Literal["authoritative", "not_applicable", "unavailable"]] = {
    ...
    packet = assemble_context_packet(
        audience=audience, records=records, lane_status=statuses,
        source_errors=supplemental_errors, budget=budget, lane_assembly_ms=timings,
        retrieval_dependencies=[
            "campaign", "campaign_thread_membership", "dm_turn", "dm_turn_attempt",
            "player_submissions", "player_submission_segments", "characters",
            "dnd5e_character_sheets", "campaign_domain_events",
        ],
    )
