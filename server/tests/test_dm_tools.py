...
class DmToolsTest(unittest.TestCase):
    ...
    def test_chat_flow_groups_visible_messages_and_nested_branches(self):
        ...
        self.assertEqual([step['kind'] for step in branch_steps], ['prompt_message', 'prompt_message', 'model_request', 'model_response', 'tool_call', 'tool_result'])
        self.assertEqual([step['category'] for step in branch_steps], ['agents', 'agents', 'agents', 'agents', 'tools', 'tools'])
        self.assertEqual(branch_steps[0]['prompt_role'], 'system')
        self.assertEqual(branch_steps[0]['content'], 'You are the test DM.')
        self.assertEqual(branch_steps[1]['prompt_role'], 'user')
        self.assertEqual(branch_steps[2]['messages'], [])
        self.assertEqual(branch_steps[4]['title'], 'get_current_scene')
        self.assertEqual(branch_steps[5]['result']['current_scene']['location_name'], 'Dock Ward')
