...
def build_session_hot_context(campaign, session, current_user, recent_messages_override=None):
    ...
    context = {
        ...
        'tool_policy': (
            ...
            'When an NPC knowingly speaks against their canonical identity, dossier secrets, or dm_private motivations '
            '(for example a saboteur delivering a cover-story), do not leak the canonical secret into the visible reply. '
            'Instead, fill resolver_packet.speaker_reliability with the public NPC name, reliability "unreliable_cover" '
            '(or unreliable_exaggeration / unreliable_omission as appropriate), and a short in-world reason. The reason '
            'is server-side metadata: keep it a brief in-world cue and do not restate the canonical secret. '
